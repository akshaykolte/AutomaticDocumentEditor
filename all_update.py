...
def read_rwc_doc_file(doc_file_name):
    try:
        text = docx2txt.process(doc_file_name)
        print 'Doc file %s found'%(doc_file_name)
    except:
    ...
    index = 0
    start_text = 'OFFICE OF THE SUPERINTENDENT OF CENTRAL EXCISE & CUSTOMS, WHAGHOLI RANGE, ALANDI DIVISION'
    end_text = 'Sub.: Issuance of Re-warehousing Certificate Report'
    address_list = []
    while index < len(text):
        start = text.find(start_text, index) + len(start_text)
        if start < len(start_text):
            break
        start = text.find('Dated:', start) + len('Dated:')
        end = text.find(end_text, start)
        if end >=0:
            address_text = text[start:end]
            address_list.append(text[start:end].strip().replace('\n\n', '\n'))
    ...
    for i in range(len(tables)):
        table = tables[i]
        table_list.append({})
        table_list[-1]['particulars'] = get_text_from_cell(table.rows[1].cells[1])
        table_list[-1]['description_of_goods'] = get_text_from_cell(table.rows[5].cells[1])
        table_list[-1]['gross_weight'] = get_text_from_cell(table.rows[6].cells[1])
        table_list[-1]['date_of_rewarehousing'] = get_text_from_cell(table.rows[11].cells[1])
        table_list[-1]['shortage'] = get_text_from_cell(table.rows[13].cells[1])
        table_list[-1]['address'] = address_list[i]
...
def save_to_ic_doc_file(doc_file_name, unit_details, table_dict):
    tpl=DocxTemplate(IC_DOCX_TEMPLATE)
    context = {}

    context['ic_date'] = unit_details['ic_date']
    context['pc_date'] = unit_details['pc_date']
    context['country_name'] = unit_details['country_name']
    context['name_of_supplier'] = unit_details['name_of_supplier']
    context['invoice_number_and_date'] = unit_details['invoice_number_and_date']

    context['description_of_goods'] = unit_details['description_of_goods']
    context['cif_value'] = 'CIF/USD: ' + ('%.2f'%(unit_details['cif_value']))

    context['email_id_and_phone_number'] = table_dict['email_id_and_phone_number']
    context['capital_goods_limit'] = table_dict['capital_goods_limit']
    context['running_balance'] = table_dict['running_balance']
...
def update_ic_file(unit_name, unit_details):
    base_unit_director = BASE_DIR + unit_name + SLASH
...
def update(file_path, directory):
    global BASE_DIR, SLASH, EXCEL_FILE, TEMPLATES_DIR, PC_DOCX_TEMPLATE, IC_DOCX_TEMPLATE, RWC_DOCX_TEMPLATE, EMPTY_FILE
    ...
    IC_DOCX_TEMPLATE = TEMPLATES_DIR + SLASH + 'IC Template - Marvel.docx'
    RWC_DOCX_TEMPLATE = TEMPLATES_DIR + SLASH + 'RWC Template - Marvel.docx'
    EMPTY_FILE = TEMPLATES_DIR + SLASH + 'empty.docx'

    unit_details = view_excel_file(BASE_DIR + EXCEL_FILE, 'Axa Marvel', 'marvel')
